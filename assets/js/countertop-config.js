    ...
    function draw() {
      ...
      const labelNumbers = (parent, cx, cy, cur, dims) => {
        ...
        if (cur.type === "rect") {
          ...
          const extra = 16; // extra gap beyond splash for label separation
          const bs = cur.bs || {};
          const aPx = px(a),
            bPx = px(b);
          // A top
        ...
        if (cur.type === "l") {
          ...
          const cMidX = !flipX
            ? cx - aPx / 2 + cPx / 2
            : cx + aPx / 2 - cPx / 2;
          const cY = cy + bPx / 2 + 22;
          mk(cMidX, cY + (bs.C ? bhpx + extra : 0), `C: ${c}\"`);
        ...
        if (cur.type === "u") {
          ...
          // A top outer width
          mk(cx, yTop - 14 - (bs.A ? bhpx + extra : 0), `A: ${a}\"`);
          // B-L and B-R numbers (left and right verticals)
          mkRot(
            cx - aPx / 2 - 30 - (bs.BL ? bhpx + extra : 0),
            yTop + blPx / 2,
            `BL: ${bl}\"`,
            -90
          );
          mkRot(
            cx + aPx / 2 + 30 + (bs.BR ? bhpx + extra : 0),
            yTop + brPx / 2,
            `BR: ${br}\"`,
            -90
          );
      ...
      shapes.forEach((cur, idx) => {
        ...
        if (shape === "rect") {
          ...
          // wall side overlays (black if against wall) — independent of backsplash
          const sideColor = "#000";
          ...
          // B: full vertical on wall side (mirror-aware)
          if (cur.wall.B) {
            if (!flipX) {
              rotG.appendChild(
                mkLine(
          ...
          // C: bottom segment of length c (mirror-aware)
          if (cur.wall.C) {
            if (!flipX) {
              rotG.appendChild(
                mkLine(
          ...
          // D: top segment on opposite vertical of height d (mirror-aware)
          if (cur.wall.D) {
            if (!flipX) {
              rotG.appendChild(
                mkLine(
          ...
          // Wall edges are independent of backsplash — use wall flags and draw exact spans
          if (cur.wall.A)
            rotG.appendChild(
