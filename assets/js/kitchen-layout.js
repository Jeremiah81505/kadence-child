(function(){
  ...
  const px=(v)=> v*2; // 2px per inch

  function init(root){
    if (!root || root.__klInit) return; root.__klInit = true;
    const svg = root.querySelector('svg.kl-svg');
    let shape = {
      type:'u', // restart with U as primary
      pos:{x:300,y:260}, rot:0,
      len:{ A:180, B:120, C:84, D:8, E:48, F:36, G:36, H:48 }
    };
    let active=true; let handles=[]; let dragging=false; let start={}; let orig={};

    function clampU(s){
      const L=s.len; const A=L.A, B=L.B;
      // D: 0..B-1
      L.D = Math.max(0, Math.min(L.D, Math.max(0,B-1)));
      // E/H: keep opening at least 1"
      L.E = Math.max(0, Math.min(L.E, Math.max(0, A-1 - Math.max(0,L.H))));
      L.H = Math.max(0, Math.min(L.H, Math.max(0, A-1 - Math.max(0,L.E))));
      L.C = Math.max(1, A - L.E - L.H);
      const maxV = Math.max(0, B - L.D - 1);
      L.F = Math.max(0, Math.min(L.F, maxV));
      L.G = Math.max(0, Math.min(L.G, maxV));
    }

    function draw(){
      clampU(shape);
      svg.innerHTML=''; handles=[];
      const g = document.createElementNS(ns,'g'); svg.appendChild(g);
      const cx=shape.pos.x, cy=shape.pos.y; const L=shape.len; const A=L.A, B=L.B;
      const a=px(A), b=px(B);
      const x=cx - a/2, y=cy - b/2;
      const xiL = x + px(L.E);
      const xiR = x + a - px(L.H);
      const yTop = y + px(L.D);
      const yBotL = yTop + px(L.F);
      const yBotR = yTop + px(L.G);
      ...
      path.setAttribute('d', d); path.setAttribute('class','kl-top'); g.appendChild(path);

      // Active outline
      const hi=document.createElementNS(ns,'rect'); hi.setAttribute('x', String(x-4)); hi.setAttribute('y', String(y-4)); hi.setAttribute('width', String(a+8)); hi.setAttribute('height', String(b+8)); hi.setAttribute('class','kl-outline'); g.appendChild(hi);

      // Guides A–H (letters only like screenshot)
      const addLine=(x1,y1,x2,y2,label)=>{ const l=document.createElementNS(ns,'line'); l.setAttribute('x1',x1); l.setAttribute('y1',y1); l.setAttribute('x2',x2); l.setAttribute('y2',y2); l.setAttribute('class','kl-guide'); g.appendChild(l); const t=document.createElementNS(ns,'text'); t.setAttribute('x', String((x1+x2)/2)); t.setAttribute('y', String((y1+y2)/2 - 6)); t.setAttribute('class','kl-text small'); t.textContent=label; g.appendChild(t); };
      const m=18; // margin inside edges
      // A top outer
      addLine(x+m, y+m, x+a-m, y+m, 'A');
      // B outer left & right
      addLine(x+m, y+m, x+m, y+b-m, 'B');
      addLine(x+a-m, y+m, x+a-m, y+b-m, 'B');
      // C inner top
      addLine(xiL+m, yTop+m, xiR-m, yTop+m, 'C');
      // D inner depth center
      addLine((xiL+xiR)/2, y+m, (xiL+xiR)/2, yTop-m, 'D');
      // E return left bottom
      addLine(x+m, y+b-m, xiL-m, y+b-m, 'E');
      // H return right bottom
      addLine(xiR+m, y+b-m, x+a-m, y+b-m, 'H');
      // F/G inner verticals
      addLine(xiL-m, yTop+m, xiL-m, yBotL-m, 'F');
      addLine(xiR+m, yTop+m, xiR+m, yBotR-m, 'G');

      // Top-centre numeric for A, left numeric for B (like screenshot)
      const text=(tx,ty,txt)=>{ const t=document.createElementNS(ns,'text'); t.setAttribute('x',tx); t.setAttribute('y',ty); t.setAttribute('class','kl-text'); t.textContent=txt; g.appendChild(t); };
      text(cx, y-10, `${A}\"`);
      const leftNum=document.createElementNS(ns,'text'); leftNum.setAttribute('x', String(x-22)); leftNum.setAttribute('y', String(cy)); leftNum.setAttribute('class','kl-text small'); leftNum.setAttribute('text-anchor','end'); leftNum.textContent=`${B}\"`; g.appendChild(leftNum);

  // Handles: outer A/B on edges; inner C/D/E/F/G/H
  const hg=document.createElementNS(ns,'g'); svg.appendChild(hg);
  const addHandle=(hx,hy,key)=>{ const c=document.createElementNS(ns,'circle'); c.setAttribute('cx',hx); c.setAttribute('cy',hy); c.setAttribute('r','6'); c.setAttribute('class','kl-handle'); c.dataset.key=key; hg.appendChild(c); handles.push({x:hx,y:hy,key}); };
      handles=[];
      const bottomY=y+b; const midTop=(xiL+xiR)/2;
      addHandle(cx, y, 'B-top');
      addHandle(x+a, cy, 'A-right');
      addHandle(x, cy, 'A-left');
      addHandle(cx, bottomY, 'B-bottom');
      addHandle(midTop, yTop, 'C');
      addHandle(midTop, yTop-12, 'D');
      addHandle(x + px(L.E)/2, bottomY, 'E');
      addHandle(x+a - px(L.H)/2, bottomY, 'H');
      addHandle(xiL, yTop + px(L.F)/2, 'F');
      addHandle(xiR, yTop + px(L.G)/2, 'G');
    }

    function getPoint(ev){ const rect=svg.getBoundingClientRect(); const x= ('touches' in ev ? ev.touches[0].clientX : ev.clientX) - rect.left; const y=('touches' in ev ? ev.touches[0].clientY : ev.clientY) - rect.top; // px
      // ViewBox is 0 0 600 520; map from element size
      const vb=svg.viewBox?.baseVal || {x:0,y:0,width:600,height:520};
      return { x: vb.x + x/rect.width*vb.width, y: vb.y + y/rect.height*vb.height };
    }

    let resizing=false, rKey=null, startLocal={};
    function worldToLocal(x,y){ return { x: x - shape.pos.x, y: y - shape.pos.y }; }

    svg.addEventListener('mousedown', (e)=>{
      const target=e.target; if (target instanceof SVGCircleElement && target.dataset.key){
        resizing=true; rKey=target.dataset.key; start=getPoint(e); startLocal=worldToLocal(start.x,start.y); orig={ pos:{...shape.pos}, len:{...shape.len} }; e.preventDefault(); return;
      }
      // drag whole
      dragging=true; start=getPoint(e); orig={pos:{...shape.pos}}; e.preventDefault();
    });
    window.addEventListener('mousemove', (e)=>{
      if (!dragging && !resizing) return; const pt=getPoint(e);
      if (resizing){
        const curLocal = worldToLocal(pt.x, pt.y);
        let dxIn = (curLocal.x - startLocal.x)/2; let dyIn = (curLocal.y - startLocal.y)/2;
        dxIn = Math.round(dxIn); dyIn = Math.round(dyIn);
        const L=shape.len; const o=orig.len;
        const clamp=(v,min,max)=> Math.min(Math.max(v,min),max);
        if (rKey==='A-right'){ L.A = Math.max(1, Math.round((o.A||0) + dxIn)); }
        else if (rKey==='A-left'){ const newA=Math.max(1, Math.round((o.A||0) - dxIn)); L.A=newA; shape.pos.x = orig.pos.x - dxIn; }
        else if (rKey==='B-top'){ const newB=Math.max(1, Math.round((o.B||0) - dyIn)); L.B=newB; shape.pos.y = orig.pos.y - dyIn; }
        else if (rKey==='B-bottom'){ L.B = Math.max(1, Math.round((o.B||0) + dyIn)); }
        else if (rKey==='C'){ const A=o.A; const newC=clamp(Math.round((o.C||0)+dxIn),1,Math.max(1,A-1)); const spare=Math.max(0,A-newC); L.E=Math.floor(spare/2); L.H=spare-L.E; L.C=newC; }
        else if (rKey==='D'){ L.D = clamp(Math.round((o.D||0)+dyIn),0,Math.max(0,(o.B||0)-1)); }
        else if (rKey==='E'){ const maxE=Math.max(0,(o.A||0)-1-(o.H||0)); L.E = clamp(Math.round((o.E||0)+dxIn),0,maxE); L.C = Math.max(1, (o.A||0)-L.E-(o.H||0)); }
        else if (rKey==='H'){ const maxH=Math.max(0,(o.A||0)-1-(o.E||0)); L.H = clamp(Math.round((o.H||0)-dxIn),0,maxH); L.C = Math.max(1, (o.A||0)-(o.E||0)-L.H); }
        else if (rKey==='F'){ const maxF=Math.max(0,(o.B||0)-1-(o.D||0)); L.F = clamp(Math.round((o.F||0)+dyIn),0,maxF); }
        else if (rKey==='G'){ const maxG=Math.max(0,(o.B||0)-1-(o.D||0)); L.G = clamp(Math.round((o.G||0)+dyIn),0,maxG); }
        draw(); return;
      }
      const dx=pt.x-start.x, dy=pt.y-start.y; shape.pos={ x: Math.round(orig.pos.x+dx), y: Math.round(orig.pos.y+dy) }; draw();
    });
    window.addEventListener('mouseup', ()=>{ dragging=false; resizing=false; rKey=null; });

    // initial viewBox and draw
    svg.setAttribute('viewBox', '0 0 600 520');
    draw();
  }

  function boot(){ document.querySelectorAll('.kl-wrap').forEach(init); }
...
(function(){
  ...
  function init(root){
    ...
    function drawGrid(){
      ...
      ctx.fillStyle = cs.getPropertyValue('--kc-surface') || '#ffffff';
      ...
      const gridColor = (cs.getPropertyValue('--kc-grid')||'#e9eef3').trim();
      const gridStrong = (cs.getPropertyValue('--kc-grid-strong')||'#d9e3ec').trim();
      for (let x=Math.floor(minX); x<=Math.ceil(maxX); x++){
        ctx.strokeStyle = (x%12===0)? gridStrong : gridColor;
        ctx.beginPath(); ctx.moveTo(x, minY); ctx.lineTo(x, maxY); ctx.stroke();
      }
      for (let y=Math.floor(minY); y<=Math.ceil(maxY); y++){
        ctx.strokeStyle = (y%12===0)? gridStrong : gridColor;
    ...
    function drawRooms(){
      ...
      for (const r of state.rooms){
        ...
        ctx.fillStyle = isSel ? 'rgba(46,125,50,.22)' : 'rgba(46,125,50,.12)';
        ctx.strokeStyle = isSel ? '#2e7d32' : (state.edge==='ogee' ? '#2e7d32' : 'rgba(0,0,0,.45)');
        ctx.lineWidth = 2/(state.scale*state.zoom);
