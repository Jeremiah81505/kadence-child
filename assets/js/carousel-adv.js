  ...
  let velocity=baseSpeed; // constant auto speed
    ...
    function update(force){
      ...
      for(let i=0;i<tiles.length;i++){
        ...
        const scale=0.98 + 0.04*closeness; // smaller delta -> sharper
        const zOffset=(scale-1)*60;
        tile.style.transform='translate3d(-50%,-50%,0) rotateY('+tileAngle+'deg) translateZ('+(r+zOffset)+'px) rotateY('+(-tileAngle)+'deg) scale('+scale+')';
      ...
      if(activeIndex>-1 && (force || activeIndex!==lastActive)){
        lastActive=activeIndex;
        const alt=tiles[activeIndex].querySelector('img')?.getAttribute('alt')||'';
        if(alt) live.textContent='Showing '+alt;
      }
    }
    function frame(now){
